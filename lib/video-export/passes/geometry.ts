...
export function applyGeometry(
  ...
  const scenes = timelineScenes.map((scene, index) => {
    ...
    const canvas = sourceScene?.content?.canvas;
    const elements = canvas?.elements;
    const slideMetrics = canvas as { viewportSize?: number; viewportRatio?: number } | undefined;
    ...
    const effects = scene.effects.map((effect) => {
      const { effect: resolved, unresolved } = resolveEffectGeometry(
        ...
        {
          sceneId: scene.id,
          viewportSize: slideMetrics?.viewportSize,
          viewportRatio: slideMetrics?.viewportRatio,
        },
      );
      if (unresolved) {
        diagnostics.push({
