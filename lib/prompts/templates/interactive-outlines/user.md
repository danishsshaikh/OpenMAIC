...
**Final reminder**: your entire response must be a JSON **object** with exactly three top-level keys — `languageDirective` (string, following the English-only language policy in the system prompt), `courseTitle` (string, ≤30 chars, in English), and `outlines` (array of scene objects). Do not return a bare array. Do not wrap in prose or code fences.
