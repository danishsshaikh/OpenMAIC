...
const TYPE_ACCENT: Record<QuizQuestionType, { badge: string; rail: string }> = {
  single: {
    ...
    rail: 'from-primary to-primary',
  },
  multiple: {
    badge: 'bg-secondary text-secondary-foreground',
    rail: 'from-[var(--brand-secondary)] to-[var(--brand-secondary)]',
  },
  short_answer: {
...
/** Brand focus treatment shared by every text input in the form. */
const FOCUS = 'focus-visible:border-primary/40 focus-visible:ring-primary/20';
  ...
  return (
    <Reorder.Item
      ...
      className={cn(
        ...
        expanded
          ? 'border-primary/25 shadow-[0_12px_32px_-16px_rgb(var(--brand-shadow)/0.32)]'
    ...
    >
      ...
      <div
        className={cn(
          ...
          expanded && 'bg-gradient-to-r from-primary/5 to-transparent',
        )}
      >
        <button
          type="button"
      ...
      {expanded && (
        ...
        >
          ...
          {!choice && (
            <>
              <div className="flex items-start gap-2.5 rounded-xl border border-primary/20 bg-primary/10 px-3 py-2.5 text-xs leading-relaxed text-primary">
