...
export interface SceneActionsOptions {
  ...
  languageDirective?: string;
  logger?: GenerationLogger;
  allowDiscussionActions?: boolean;
}
...
export async function generateWidgetContent(
  ...
  let widgetConfigResult = validateWidgetConfigForGeneratedHtml(html, widgetType, outline, log);
  ...
  if (widgetType === 'simulation') {
    ...
    if (languageSafeHtml !== html) {
      ...
      widgetConfigResult = validateWidgetConfigForGeneratedHtml(
        acceptedHtml,
        widgetType,
        outline,
        log,
...
function validateWidgetConfigForGeneratedHtml(
  ...
  const widgetConfigResult = extractWidgetConfigResult(html, widgetType);
...
function extractWidgetConfigJson(html: string): string | undefined {
  SCRIPT_TAG_RE.lastIndex = 0;
  for (const match of html.matchAll(SCRIPT_TAG_RE)) {
    const attrs = match[1] ?? '';
    if (
      /\bid\s*=\s*["']widget-config["']/i.test(attrs) &&
 ...
 */
function extractWidgetConfigResult(
  html: string,
  widgetType: WidgetType,
): WidgetConfigExtractionResult {
  const widgetConfigJson = extractWidgetConfigJson(html);
  if (!widgetConfigJson) return { status: 'missing' };

  try {
    const parsed: unknown = JSON.parse(widgetConfigJson);
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
...
export function extractWidgetConfig(
  ...
  const widgetConfigJson = extractWidgetConfigJson(html);
  if (!widgetConfigJson) return undefined;
...
export async function generateSceneActions(
  ...
  if (outline.type === 'slide' && 'elements' in content) {
    ...
    const actions = parseActionsFromStructuredOutput(
      response,
      outline.type,
      options.allowDiscussionActions === false ? SLIDE_ACTIONS_WITHOUT_DISCUSSION : undefined,
      log,
    );

    if (actions.length > 0) {
      // Validate and fill in Action IDs
      return processActions(actions, content.elements, agents, log, options.allowDiscussionActions);
    }
  ...
  if (outline.type === 'quiz' && 'questions' in content) {
    ...
    if (actions.length > 0) {
      return processActions(actions, [], agents, log, options.allowDiscussionActions);
    }
...
function processActions(
  ...
  return actions.flatMap((action) => {
    if (action.type === 'discussion' && !allowDiscussionActions) {
      return [];
    }
